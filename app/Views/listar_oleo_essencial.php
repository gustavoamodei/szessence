<div class="container">
    <div class="row  align-self-center d-flex justify-content-center">  
        ...
        <?php if($session->getFlashdata('msg')){?>
            ...
            <a href="<?= base_url('cadastro_oleo')?>" class="btn btn-success mb-3">Cadastrar</a>
            <h5 class="align-self-center d-flex justify-content-center">Óleos Essenciais<h5>
            <div class="table-responsive">
                <table class=" table table-sm table-bordered table-hover" id="table_oleo_essencial">
                    <thead class="thead-dark">
                        ...
                        <th scope="col">Gotas atual</th>
                        <th scope="col">Gotas anterior</th>
                    ...
                    <tbody>
                            ...
                            <?php  foreach($query as $oe){ ?>
                                    <tr>
                                        <td><?=$oe['id']?></td>
                                        <td><?=$oe['nome']?></td>
                                        <td><?=$oe['ml'] ?></td>
                                        <td><?=number_format($oe['preco_gota'], 2 , ",", "."); ?></td>
                                        <td><?=$oe['total_gotas'] ?></td>
                                        <td><?=$oe['t_gotas_antes'] ?></td>
                                        <td><?=number_format($oe['valor_compra'], 2 , ",", ".");?></td>
                                        <td><?=date('d/m/Y',strtotime($oe['validade']))?></td>
                                        <td>
                                            <a href="<?= base_url('edit_oleo/'.$oe['id']) ?>" class="btn btn-primary">Edit</a>
...
<script>
    $(document).ready( function (){
        $('#table_oleo_essencial').DataTable(
                {"bJQueryUI": true,
                    "oLanguage": {
        ...
        $(document).on("mouseenter click","#table_oleo_essencial>tbody>tr",function(){
        let dados = $('#table_oleo_essencial').DataTable().row(this).data();
        $("#id_oe").text(dados[0]);
    });
        ...
        $.ajax({
                ...
                success:function(){
                    $('#alert_red').html( 
                        ...
                        Oleo Essencial Deletado com sucesso!!
