<div class="container">
        ...
        <?php if($session->getFlashdata('msg')){?>
            ...
            <a href="<?= base_url('cadastro_acessorio')?>" class="btn btn-success mb-3">Cadastrar</a>
            <h5 class="align-self-center d-flex justify-content-center">Acessórios<h5>
            <table class=" table table table-bordered table-hover" id="table_acessorio">
                ...
                <tbody>
                        ...
                        <?php  foreach($query as $acessorio){ ?>
                                <tr>
                                    <td><?=$acessorio['id']?></td>
                                    <td><?=$acessorio['nome']?></td>
                                    <td><?=number_format($acessorio['valor'], 2 , ",", "."); ?></td>
                                    <td><?=$acessorio['descricao']?></td>
                                    <td>
                                        <a href="<?= base_url('edit_acessorio/'.$acessorio['id']) ?>" class="btn btn-primary">Editar</a>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    Deseja Excluir Este registro de ID:<span id="id_acessorio"></span>?
                </div>
...
<script>
    $(document).ready( function (){
        ...
        $(document).on("mouseenter click","#table_acessorio>tbody>tr",function(){
        let dados = $('#table_acessorio').DataTable().row(this).data();
        $("#id_acessorio").text(dados[0]);
    });


    $("#deletar").click(function(){
        var id=$("#id_acessorio").text();
      
        $.ajax({
                type:"post",
                url:'<?=base_url()?>/delete_acessorio',
                data:{id:id},
                success:function(){
                    $('#alert_red').html( 
                        ...
                        Acessório Deletado com sucesso!!
